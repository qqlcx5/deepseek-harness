...
| `@deepseek-ai/dsh-tool-objective` | `attach_objective`、`create_objective`、`get_objective`、`list_objectives` | `ctx.tools`、`ctx.agents`、`ctx.objectives`、`ctx.systemPrompt`、`a calling Agent in an authorized open turn` | `tool/call`、`tool/result`、`objective/member mirrored on attach` | - | list 和 get 对任意调用 agent 可读；create 和 attach 要求直接来自人类的根权限。结果携带缓存综述 brief，模型无需重读成员会话即可回答意图状态。 |
...
<a id="deepseek-aidsh-tool-objective"></a>

## `@deepseek-ai/dsh-tool-objective`

### `attach_objective`

将当前会话挂载到一个既有 objective 上，记录该会话服务于这一意图。优先在人类确认会话归属后立即提议。要求直接人类轮。
...
来源：[`packages/objective/tool-objective/src/index.ts`](../packages/objective/tool-objective/src/index.ts)

### `create_objective`

当直接人类请求命名了一个将跨越多个会话的持久意图——比本次对话更大的目标——时，创建一个跨会话 objective。不要用于单会话工作（那属于同会话 goal 工具）。要求直接人类轮。
...
完整读取一个 objective：标题、北极星陈述、状态、带时间戳的缓存综述 brief，以及有序的成员会话。把 brief 当作当前跨会话结论使用，而不是重读每个成员会话。
...
按注册表顺序列出每个跨会话 objective 的状态、成员会话数，以及是否存在综述 brief 与北极星陈述。创建 objective 前先读它，避免重复，并找到可挂载当前会话的既有 objective。
...
list 和 get 对任意调用 agent 可读；create 和 attach 要求直接来自人类的根权限。结果携带缓存综述 brief，模型无需重读成员会话即可回答意图状态。
