...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  {
    pkg: '@deepseek-ai/dsh-tool-objective',
    dir: 'tool-objective',
    source: 'packages/objective/tool-objective/src/index.ts',
    requires: ['ctx.tools', 'ctx.agents', 'ctx.objectives', 'ctx.systemPrompt', 'a calling Agent in an authorized open turn'],
    writes: ['tool/call', 'tool/result', 'objective/member mirrored on attach'],
    async mount(ctx) {
      await ctx.plugin(AgentRegistry)
      // Catalog boot needs no medium: an in-memory domain stand-in keeps the
      // registry off disk while exercising the real service and tools.
      const records = new Map<string, unknown>()
      let globalState: unknown = { objectiveIds: [] }
      /* oxlint-disable typescript/require-await -- async signatures match the storage domain contract; the stand-in never awaits */
      const table = {
        size: records.size,
      ...
      ctx.provide('storageDomain', {
        open: async () => ({
          ...
          global: {
            get: () => globalState,
            set: async (value: unknown) => { globalState = value },
          },
          close: async () => {},
        }),
      } as never)
      ...
      await ctx.plugin(ToolObjective)
    },
    note:
      'list and get are reads for any calling agent; create and attach require direct-human root authority. Results carry the cached synthesis brief so a model answers intent status without re-reading member sessions.',
  },
  {
