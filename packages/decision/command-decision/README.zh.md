...
`/decide` 不带参数列出全部决策(`[O]`/`[D]`/`[S]` 标志、已决定行显示所选选项、短 id 片段)加到期提醒与用法。裸的非关键字输入创建 open 决策。`show <id>` 渲染完整卡片——带证据/代价/风险的选项、带置信度的推荐、**始终渲染的反证段**(空段显式说明;缺席从不静默)、决定后的所选选项与冻结预测、可逆性。`choose <id> <option>` 经注册表的卡片校验拍板;不可逆决策的第一遍返回确认块(当前反证段、选项证据与一个 stamp),只有 `choose <id> <option> confirm <stamp>` 才生效——卡被改动会使 stamp 失效并要求重读。目标链接悬空或目标已 parked 的行带 `(!)` 标记(objective 组未组合时静默);`rev <id> <outcome>` 基于冻结预测记录校准行;`super <id>` 与 `delete <id>` 收尾(删除保留回访轨迹)。id 片段在稳定 id 内任意位置匹配;歧义片段列出命中并要求更多字符。
...
- **证据按草稿渲染** — 知识层回填 claim id 之前,选项证据是自由文本;卡片不校验它。
