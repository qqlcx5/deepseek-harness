...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'objectives',
    summary: 'Objective registry (`ctx.objectives`) over the storage domain form.',
    description: 'Objective registry (`ctx.objectives`) over the storage domain form. Startup opens the domain and validates that registry order and records agree; every mutation runs on a serialized write chain and publishes its notification only after the durable write succeeds. Membership writes the domain first; the session-log mirror is best-effort for live sessions and never rolls a committed domain write back.',
    methods: [
      {
        signature: 'async create(request: CreateObjectiveRequest): Promise<ObjectiveView>',
        description: 'Create an active objective and prepend it to the durable registry order.',
        parameters: [{ name: 'request', description: 'Title and optional north-star statement.' }],
        returns: 'the created view.',
      },
      {
        signature: 'get(id: ObjectiveId): ObjectiveView | undefined',
        description: 'Look up an objective by id.',
        parameters: [{ name: 'id', description: 'Objective id.' }],
        returns: 'the detached view, or `undefined` when unknown.',
      },
      {
        signature: 'list(): ObjectiveView[]',
        description: 'Synchronous objective projection in durable registry order. Performs no persistence reads.',
        parameters: [],
        returns: 'a fresh ordered array of detached views.',
      },
      {
        signature: 'objectivesOf(sessionId: SessionId): ObjectiveView[]',
        description: 'List the objectives one session currently serves.',
        parameters: [{ name: 'sessionId', description: 'Member session id.' }],
        returns: 'the member objectives in durable registry order.',
      },
      {
        signature: 'async update(id: ObjectiveId, request: UpdateObjectiveRequest): Promise<ObjectiveView>',
        description: 'Edit title, north-star statement, and/or status. Every status transition is legal (close is not terminal: reopening an objective is a supported flow); an edit changes no other field.',
        parameters: [{ name: 'id', description: 'Objective id.' }, { name: 'request', description: 'At least one replacement field; `null` northStar clears it.' }],
        returns: 'the updated view.',
      },
      {
        signature: 'async attachSession(id: ObjectiveId, sessionId: SessionId): Promise<ObjectiveView>',
        description: 'Record that a session serves this objective. Idempotent: an already accounted id resolves without writing. The domain row commits first; a live member session additionally receives a log-only `objective/member` event, whose failure is logged and never rolls the domain write back. A session that is neither live nor persisted is accepted the same way — the domain is the membership authority, the log mirror is display data.',
        parameters: [{ name: 'id', description: 'Objective id.' }, { name: 'sessionId', description: 'The member session to record.' }],
        returns: 'the updated view.',
      },
      {
        signature: 'async detachSession(id: ObjectiveId, sessionId: SessionId): Promise<ObjectiveView>',
        description: 'Remove a session from this objective\'s member account. Idempotent: an unknown member resolves without writing. A live member session receives the matching `objective/member` detach event under the same best-effort rule as attachSession.',
        parameters: [{ name: 'id', description: 'Objective id.' }, { name: 'sessionId', description: 'The member session to remove.' }],
        returns: 'the updated view.',
      },
      {
      ...
      {
        signature: 'async delete(id: ObjectiveId): Promise<boolean>',
        description: 'Delete one objective record while retaining every member session log (historic `objective/member` events stay and are harmless: their objective simply no longer resolves). The durable order is updated before the record deletion; a failed record write restores the prior order. A crash between the two writes leaves an orphan record that the next startup rejects loudly.',
        parameters: [{ name: 'id', description: 'Objective to remove.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    mode: 'emit',
    signature: '\'objective/changed\'(payload: ObjectiveChanged): void',
    summary: 'One durable objective mutation committed.',
    description: 'One durable objective mutation committed.',
    parameters: [{ name: 'payload', description: '.objective - post-mutation view; absent for a delete.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'CreateObjectiveRequest',
    declaration: 'export interface CreateObjectiveRequest {\n    readonly title: string;\n    readonly northStar?: string;\n}',
  },
  {
  ...
  {
    name: 'ObjectiveChanged',
    declaration: 'export interface ObjectiveChanged {\n    readonly operation: ObjectiveOperation;\n    readonly objective?: ObjectiveView;\n}',
  },
  {
    name: 'ObjectiveOperation',
    declaration: 'export type ObjectiveOperation = \'create\' | \'update\' | \'attach\' | \'detach\' | \'brief\' | \'delete\';',
  },
  {
    name: 'ObjectiveStatus',
    declaration: 'export type ObjectiveStatus = \'active\' | \'parked\' | \'closed\';',
  },
  {
    name: 'ObjectiveView',
    declaration: 'export interface ObjectiveView {\n    readonly id: ObjectiveId;\n    readonly title: string;\n    readonly northStar?: string;\n    readonly status: ObjectiveStatus;\n    readonly brief?: string;\n    readonly briefAt?: string;\n    readonly sessionIds: readonly SessionId[];\n    readonly createdAt: string;\n    readonly updatedAt: string;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface UpdateObjectiveRequest {\n    readonly title?: string;\n    readonly northStar?: string | null;\n    readonly status?: ObjectiveStatus;\n}',
  },
  {
    name: 'UserMessage',
