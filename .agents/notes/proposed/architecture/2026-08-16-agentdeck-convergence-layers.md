...
- Landed so far: the registry (`dsh-objective`), the tools (`dsh-tool-objective`), the command with the WIP signal (`dsh-command-objective`), the synthesizer (`dsh-objective-synthesizer`), and a keyless assembled snapshot. The auto-attribution consumer and the unassigned view are not yet built; until they land this is a brief layer, not the full intent layer. The decision layer (Phase B) is fully landed: `dsh-decision`, `dsh-command-decision`, `dsh-decision-drafter`, with the irreversible confirming pass, the stale-card fence, and objective-level review aggregation.
...
- Promotion requires corroboration from distinct evidence sources — distinct at canonical document granularity, so two anchors inside one artifact never count twice (`sourceUri`-deduplicated to document roots, at least two): parallel audits routinely share one upstream artifact, and six retellings of one wrong fact are one wrong fact, not six confirmations. A demotion propagates back to the promoted memory entry. When the sentinel supersedes a claim cited by a decided decision, that decision is marked for re-review.
...
- Reversibility triages the approval path: a reversible option follows the fast path and may authorize the next delegated step; an irreversible one requires an explicit confirming second pass that quotes the card's current counter-evidence and a stamp — a stale stamp (the card changed under the reader) rejects — and never carries execute authorization.
...
- A keyless assembled check exists per phase (multi-session objective with brief via scripted replay; the decision lifecycle — counter-evidence, the irreversible confirming pass, review, objective aggregation — via a real-Loader boot smoke; extracted claim with a detected and adjudicated contradiction), through real runnable examples per the testing policy.
